"""
Extraction des métadonnées structurées via LLM (llama3.2 ou Qwen).
"""
import re
import time
...
from core.agents import (
    ...
    image_to_base64, USE_OLLAMA, USE_QWEN,
)
...
DOC_FIELD_SCHEMAS = {
    "carte_identite": {
        ...
        "fields": {
            "nom":"Nom de famille","prenom":"Prénom(s)","cin_number":"Numéro CIN",
            "date_naissance":"Date de naissance","lieu_naissance":"Lieu de naissance",
            "date_delivrance":"Date de délivrance","date_expiration":"Date d'expiration",
            "adresse":"Adresse complète","sexe":"Sexe (M/F)",
        }
    },
    "rib": {
    ...
    "acte_heredite": {
        ...
        "fields": {
            ...
            "notaire":"Notaire","date_acte":"Date de l'acte","lieu_acte":"Lieu de l'acte",
        }
    },
    "assurance": {
        "label": "Contrat d'assurance",
        "fields": {
            "assure":"Assuré(e)","num_contrat":"N° contrat","type_garantie":"Type de garantie",
            "date_effet":"Date d'effet","date_echeance":"Date d'échéance",
            "prime":"Prime/cotisation","assureur":"Compagnie d'assurance","agence":"Agence",
        }
    },
    "attestation_solde": {
...
def extract_metadata_with_llm(ocr_text: str, doc_type: str, stored_path: str = "") -> dict:
    """
    Extrait les métadonnées via streaming Ollama.
    Stratégie : llama3.2 → Qwen2.5-VL → résultat vide.
    """
    ...
    if USE_OLLAMA:
        ...
        print(f"[Extract] llama3.2 → {len(raw)} chars en {time.time()-t0:.0f}s")
        if raw:
    ...
    if USE_QWEN and stored_path and __import__("os").path.exists(stored_path):
        prompt_q = (
        ...
        try:
            ...
            print(f"[Extract] Qwen → {len(raw)} chars en {time.time()-t0:.0f}s")
            if raw:
                result = _parse_llm_json(raw, field_keys)
                if result and any(v for v in result.values()):
                    result["_source"] = "qwen"
    ...
    print("[Extract] ❌ ÉCHEC — retourne null")
    return empty_result